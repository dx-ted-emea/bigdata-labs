...
2.	As a best practice, each Stream Analytics job should read from a single Event Hubs Consumer Group. We will walk you through the process of creating a Consumer Group below.  To create a Consumer Group, navigate to the newly created Event Hub and click the **CONSUMER GROUPS** tab, then click **CREATE** on the bottom of the page and provide a name for your Consumer Group. If you don't create a Consumer Group, you can use the "$Default" group.
...
4.	Under **SHARED ACCESS POLICIES**, create a new policy with **MANAGE** permissions called, for example, "manage".
![Shared Access Policies where you can create a policy with Manage permissions.](./media/stream-ananlytics-shared-access-policies.png)
...
![Connection Information where you can get your connection strings.](./media/connection-nformation.png)
...
1.	[Download the Movies TwitterClient solution](https://github.com/rachelye/Azure-BigData-labs/tree/master/Lab-1)
...
[Steps to generate an OAuth access token](https://dev.twitter.com/oauth/overview/application-owner-access-tokens). Note that you will need to make an empty application to generate a token.
...
1.	Select **DATA STREAM**, and then click the right button.
1.	Select **EVENT HUB**, and then click the right button.
1.	Type or select the following values on the third page:
...
* **EVENT HUB POLICY NAME**: Select the event-hub policy created earlier in this tutorial (we called it "manage").
* **EVENT HUB CONSUMER GROUP**: Type in the Consumer Group created earlier in this tutorial (or use the "$Default").
...
1.	Click the check button to add this source and to verify that Stream Analytics can successfully connect to the event hub.
